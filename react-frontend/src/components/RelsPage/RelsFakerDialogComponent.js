...
import relsFakerFactory from "./relsFakerFactory";
...
const RelsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = relsFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("rels").create(elem));
...
export default connect(null, mapDispatch)(RelsFakerDialogComponent);
