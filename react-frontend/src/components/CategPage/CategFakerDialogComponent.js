...
import categFakerFactory from "./categFakerFactory";
...
const CategFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = categFakerFactory(fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("categ").create(elem));
...
export default connect(null, mapDispatch)(CategFakerDialogComponent);
